...
ListOfObjRazRules *s57chart::GetObjRuleListAtLatLon(float lat, float lon, float select_radius, ViewPort *VPoint)
{
            ...
            case  GEO_POINT:
                ...
                //  possibly expanded by text or symbol rendering
                {
                    if(1 == obj->npt)
                    {
                          ...
                          {
                                double olon, olat;
                                fromSM((obj->x * obj->x_rate) + obj->x_origin, (obj->y * obj->y_rate) + obj->y_origin, ref_lat, ref_lon, &olat, &olon);
